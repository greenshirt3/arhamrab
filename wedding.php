...
<body>
    ...
    <a class="btn-sm-square bg-white text-dark rounded-circle me-4 position-relative" href="https://shop.arhamprinters.pk/#cart-section">
    ...
    <a href="#" class="mobile-nav-link active" onclick="showSection('shop-catalog')" id="mobile-link-wedding">
                ...
                <div class="row g-5">
                    <div class="col-lg-6">
                        <div id="cardImageGallery" class="carousel slide" data-bs-ride="carousel">
                            <div class="carousel-indicators-wrapper" id="carousel-indicators"></div>
...
<li><a class="text-white" href="https://return.arhamprinters.pk/">Return Policy</a></li>
    ...
    let hasInner = false; 
    let visibleCards = {}; // Store visibility state per category
    ...
    function detectLocation() {
        document.getElementById('zone-name').textContent = "Please enter your city name above for zone check.";
        if (typeof Cart !== 'undefined' && typeof Cart.showAlert === 'function') {
             Cart.showAlert("Please enter your city manually.", 'info');
        }
    }
    ...
    function updateShippingZone(cityName) {
        ...
        if (normalizedCity.includes('jalalpur') || normalizedCity.includes('jattan') || normalizedCity === 'jlp') {
            zone = 'Within City';
        }
        ...
        zoneDisplay.classList.add(zone === 'Within City' ? 'text-success' : 'text-danger');
    ...
    async function initialAppLoad() {
        ...
        try {
            ...
            if (spinner) {
                 spinner.innerHTML = `<p class="alert alert-danger">Error: Failed to load card data.</p>`;
    ...
    function initialCardSetup(flatData) {
        ...
        if (!tieredWeddingData || Object.keys(tieredWeddingData).length === 0) {
            document.getElementById('wedding-card-grid').innerHTML = '<p class="alert alert-danger">Error: Wedding card data structure missing or empty.</p>';
            return;
        }
    ...
    function calculateFinalPrice(quantity) {
        ...
        if (currentDeliveryType === 'SelfPickUp') {
            ...
            if (currentShippingZone === 'Within City') {
                // Option 2: Local Fixed 100
                shippingCost = 100;
                finalShippingNote = `Local Delivery (Jalalpur Jattan)`;
            } else {
                // Option 3: Non-Local (Added Later)
                shippingCost = 0;
                isShippingPending = true;
                finalShippingNote = `Courier charges will be added to bill later via WhatsApp.`;
            }
        }
    ...
    // UI UPDATES
    function showAllCards(categoryName) {
        const safeCategoryName = categoryName.replace(/\s/g, '-');
        document.querySelectorAll(`.card-category-${safeCategoryName}`).forEach(card => {
            card.classList.remove('card-hidden');
        });
        const button = document.getElementById(`view-all-btn-${safeCategoryName}`);
    ...
    function updateQuantityAndPrice(change = 0) {
        ...
        if (currentCard && quantity >= MIN_QTY) {
            ...
            const shipCostDisplay = document.getElementById('shipping-cost-display');

            if (result.isShippingPending) {
                // Show "Price + Delivery"
                priceDisplay.innerHTML = `PKR ${result.baseProductPrice.toLocaleString()} <span class="fs-6 text-warning">+ Delivery</span>`;
                shipCostDisplay.textContent = "Calculated Later";
                shipCostDisplay.classList.add('text-warning');
            } else {
                // Show Exact Total
                priceDisplay.textContent = `PKR ${result.finalPrice.toLocaleString('en-PK', { minimumFractionDigits: 0 })}`;
                shipCostDisplay.textContent = result.shippingCost === 0 ? "FREE" : `PKR ${result.shippingCost}`;
                shipCostDisplay.classList.remove('text-warning');
            }
    ...
    function addItemToCart() {
        ...
        const cartItem = {
            ...
            finalItemPrice: result.baseProductPrice, 
            options: result.options,
            isAreaBased: false,
            imageFile: (currentCard.galleryImages ? currentCard.galleryImages[0] : currentCard.imageFile || 'img/wedding/placeholder.webp').match(/[^/]+(?=\.)/)[0], 
        };
    ...
    function loadWeddingCards() {
        ...
        if (Object.keys(categorizedCards).length === 0) {
             cardGrid.innerHTML = '<p class="alert alert-warning">No card designs available. Please contact us.</p>';
             return;
        }
        ...
        Object.keys(categorizedCards).forEach(category => {
            ...
            let remainingCards = [];
            cards.forEach(card => {
                if (card.isFeatured === true) featuredCards.push(card);
                else remainingCards.push(card);
            });
            
            const prioritizedCards = featuredCards.concat(remainingCards);
            const totalCardCount = prioritizedCards.length;
            ...
            prioritizedCards.forEach((card, index) => {
                ...
                const isHidden = index >= MAX_INITIAL_CARDS;
                const hiddenClass = isHidden ? 'card-hidden' : '';

                cardHTML += `
                    <div class='col-lg-3 col-6 mb-4 card-category-${safeCategoryName} ${hiddenClass}'>
            ...
            if (totalCardCount > MAX_INITIAL_CARDS) {
                 cardHTML += `
                     <div class='col-12 text-center my-4' id="view-all-btn-${safeCategoryName}">
                         <button class="btn btn-secondary py-3 px-5 rounded-0" onclick="showAllCards('${category}')">
                             View All ${totalCardCount - MAX_INITIAL_CARDS} Designs in ${category} <i class="fas fa-chevron-down ms-2"></i>
                         </button>
                     </div>
                 `;
            }
    ...
    function handleContactSubmission(e) {
        ...
        let whatsappMessage = `*NEW CONTACT MESSAGE (WEB - WEDDING)*\n*Sender:* ${name}\n*Subject:* ${subject}\n*Message:* ${message}`;
        window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(whatsappMessage)}`, '_blank');
        if (typeof Cart !== 'undefined' && typeof Cart.showAlert === 'function') {
