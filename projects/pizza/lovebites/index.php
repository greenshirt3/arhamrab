...
<body>
    ...
    if(!n || !p || !a) return alert("Please fill all details (Name, Phone, Address)");

    const btn = document.querySelector('.btn-success'); // Your checkout button class
    ...
    const total = subtotal + CFG.fee; // Adds delivery fee

    // 3. Send to RiderGo API
    fetch('https://ridergo.arhamprinters.pk/api/place_order.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            shop_id: "lovebitespizza",           // Must match the ID in Step 1
            shop_name: "Love Bites Pizza",
            customer_name: n + " (" + p + ")",
            cart: cart,
            total: total,
            note: a                   // Sending address as the note
        })
    })
    .then(res => res.json())
    .then(data => {
        if(data.status === 'success') {
            // Success! Redirect to WhatsApp with Order ID
            let msg = `*NEW ORDER #${data.order_id}* %0aName: ${n} %0aAddress: ${a} %0a----------------%0a`;
            cart.forEach(i => msg += `${i.qty}x ${i.name} ${i.var||''} @ ${i.price} %0a`);
            msg += `----------------%0aTotal: ${CFG.curr}${total}`;
    ...
    .catch(err => {
        console.error(err);
        alert("Connection to RiderGo failed");
        btn.innerHTML = originalText;
        btn.disabled = false;
    });
}
